...
{
  printf("Start MyCPPythonAdaptor Initialization! \n");
  if (!Superclass::CoProcessor)
  {
    printf("Superclass if in MyCPPythonAdaptor Initialization! \n");
    Superclass::CoProcessor = vtkCPProcessor::New();
    printf("Superclass if MPI Comm in MyCPPythonAdaptor Initialization! \n");
    myCPPythonAdaptorAPI::Comm = new vtkMPICommunicatorOpaqueComm(handle);
    printf("Superclass if coprocessor in MyCPPythonAdaptor Initialization! \n");
    Superclass::CoProcessor->Initialize(*myCPPythonAdaptorAPI::Comm);
  }
  // needed to initialize vtkCPDataDescription.
  printf("Coprocessor in MyCPPythonAdaptor Initialization! \n");
